...
export default function Services() {
  const { services, products, addService, updateService, consumeStock, profile, authed } = useApp();
  const canManage = profile?.role === "admin";
  const canCreate = true; // Everyone can create services now
  ...
  const submit = () => {
    ...
    if (!mechanic) { toast.error("Please specify the service provider's name (mechanic)"); return; }
